...
def main():
    # dataset
    dataset_name = choose_dataset_interactive()
    ...
    families_passing_carrie = {fam: res for fam, res in carrie_index_indiv.items() if res.carrie_measure_pass}
    print(f"Families passing Carrie measure: {len(families_passing_carrie)}")
    # plot the results
    fig, axs = plt.subplots(1, 2, figsize=(12, 6))
    fig.suptitle(f"Classification results for {classifier_name} on {dataset_name}\n{parameters}")
    style = MarkerStyle("+").scaled(0.3)
    # plot all asteroids in dataset in light/translucent gray
    for ax in axs:
        ax.scatter(proper_elements["a"], proper_elements["sinI"], color="lightgray", alpha=0.2, label="All asteroids", marker=style)
    ...
    for fam, res in families_passing_carrie.items():
        pred_fam_members = family_membership[family_membership["family1"] == fam]["name"]
        pred_fam_proper_elements = proper_elements[proper_elements["Name"].isin(pred_fam_members)]
        axs[0].scatter(pred_fam_proper_elements["a"], pred_fam_proper_elements["sinI"], label=fam, marker=style)
        true_fam_members = family_membership[family_membership["family1"] == res.corresponding_true_family]["name"]
        true_fam_proper_elements = proper_elements[proper_elements["Name"].isin(true_fam_members)]
    ...
    for i, true_threshold in enumerate(true_thresholds):
        for j, false_threshold in enumerate(false_thresholds):
            ...
            carrie_results_grid[i, j] = carrie_index_total
