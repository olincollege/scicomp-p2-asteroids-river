...
class HDBSCANFamilyClassifier(FamilyClassifier):
    """
    HDBSCAN based asteroid family classifier.
    """
    def __init__(self, min_cluster_size: int = 5, min_samples: int = None, cluster_selection_epsilon: float = 0.0, n_jobs: int = -1):
        """
        Initialize the HDBSCANFamilyClassifier with the given parameters.
    ...
    def classify(self, elements: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        results = self.hdbscan.fit_predict(elements[["a", "e", "sinI", "n", "g", "s"]])
